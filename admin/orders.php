    ...
    <div class="main-content">
        ...
        <div class="container-fluid">
            ...
            foreach ($cards as $status => [$title, $headerClass]) {
                $collapseId = $status . 'Collapse';
                $isPending = ($status === 'pending'); // Check if the current card is for pending bookings
                $showClass = $isPending ? 'show' : ''; // Add 'show' class only for pending bookings to make it expanded by default

                echo "
                <div class='card'>
                    <div class='card-header $headerClass' data-bs-toggle='collapse' data-bs-target='#$collapseId' style='cursor: pointer;'>
                        <h5 class='mb-0'>$title</h5>
                        <button class='btn btn-light btn-sm' onclick=\"event.stopPropagation(); exportTableToCSV('{$status}-bookings.csv', '{$status}BookingsTable')\">
                            <i class='fas fa-download'></i> Export
                        </button>
                    </div>
                    <div class='card-body collapse $showClass' id='$collapseId'>
