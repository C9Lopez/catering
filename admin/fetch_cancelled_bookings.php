...
$whereClause = "WHERE eb.booking_status = 'cancelled'";
$params = [];
...
try {
    $stmt = $db->prepare("
        SELECT eb.booking_id, eb.location, eb.event_date, eb.event_time, eb.setup_time, eb.number_of_guests, eb.total_amount, eb.booking_status,
               ...
               COALESCE(cp.category, CASE WHEN eb.event_type LIKE '%Children%' THEN 'Childrens Party Catering' WHEN eb.event_type LIKE '%Corporate%' THEN 'Corporate Catering' ELSE 'N/A' END) AS package_category,
    ...
    if (empty($bookings)) {
        echo "<tr><td colspan='10' class='text-center text-muted'>No cancelled bookings found.</td></tr>";
    } else {
        foreach ($bookings as $row) {
            ...
            $isCustom = (strtolower($row['package_name']) === 'custom wedding package' || strtolower($row['package_name']) === 'custom debut package' || strtolower($row['package_name']) === 'custom children\'s party package' || strtolower($row['package_name']) === 'custom corporate package');
            $customDetails = '';
            if ($isCustom) {
    ...
    error_log("Error fetching cancelled bookings: " . $e->getMessage());
