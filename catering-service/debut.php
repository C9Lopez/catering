...
try {
    $stmt = $db->prepare("SELECT * FROM catering_packages WHERE category = 'Debut Catering'");
    $stmt->execute();
    $debut_packages = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $packages_json = json_encode($debut_packages);
    
    $itemsPerPage = 2;
    $totalPackages = count($debut_packages);
...
// Fetch the package_id for Custom Debut Package
$custom_package_id = null;
foreach ($debut_packages as $pkg) {
    if (strtolower($pkg['name']) === 'custom debut package') {
        $custom_package_id = $pkg['package_id'];
        break;
    }
}

// Fetch active debut menu items for custom package
try {
    $menus_stmt = $db->prepare("SELECT m.*, c.category_name FROM menus m LEFT JOIN menu_categories c ON m.category_id = c.category_id WHERE m.service_type = 'debut' AND m.status = 'active' ORDER BY c.category_name, m.title");
    $menus_stmt->execute();
    $debut_menus = $menus_stmt->fetchAll(PDO::FETCH_ASSOC);
    
    // Fix image paths
    foreach ($debut_menus as &$menu) {
    ...
    $debut_menus_json = json_encode($debut_menus);
} catch (PDOException $e) {
    error_log("Error fetching debut menus: " . $e->getMessage());
    $debut_menus = [];
    $debut_menus_json = json_encode([]);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Debut Catering - Pochie Catering</title>
    ...
    <style>
        /* Exact styles copied from Wedding.php */
        .custom-menu-card {
            border-radius: 12px;
        ...
        /* Package Card Styles - Same as Wedding but with Debut Gradient */
        .package-image {
            height: 200px;
            object-fit: cover;
            width: 100%;
        ...
        .package-header {
            /* Pink/Purple gradient for Debut Theme */
...
<body class="light-theme">
    ...
    <div class="container-fluid py-6 wow fadeInUp" data-wow-delay="0.3s">
        <div class="container">
            <div class="row g-5 align-items-center">
                <div class="col-lg-6">
                    <h1 class="display-5 mb-4">Debut Catering Excellence</h1>
                    <p class="mb-4">
                        Your debut is a once-in-a-lifetime celebration marking your entrance into adulthood. At Pochie Catering, 
                        we specialize in creating magical debut celebrations with delectable cuisine and impeccable service.
                    </p>
                    <div class="row g-4">
                        <div class="col-md-6"><div class="d-flex align-items-center mb-3"><i class="fas fa-check text-primary me-3"></i><span>Themed Menu Planning</span></div></div>
                        <div class="col-md-6"><div class="d-flex align-items-center mb-3"><i class="fas fa-check text-primary me-3"></i><span>Professional Staff</span></div></div>
                        <div class="col-md-6"><div class="d-flex align-items-center mb-3"><i class="fas fa-check text-primary me-3"></i><span>Decorative Setup</span></div></div>
                        <div class="col-md-6"><div class="d-flex align-items-center mb-3"><i class="fas fa-check text-primary me-3"></i><span>Custom Packages</span></div></div>
                    </div>
                </div>
                <div class="col-lg-6 wow zoomIn" data-wow-delay="0.5s">
                    <img src="../img/event-4.jpg" class="img-fluid rounded" alt="Debut Catering">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="container-fluid py-6 wow fadeInUp" data-wow-delay="0.3s">
        <div class="container">
            <div class="text-center mb-5 wow fadeInUp" data-wow-delay="0.3s">
                <h1 class="display-4 text-title fw-bold">Debut Packages</h1>
                <p class="fs-5 text-subtitle">Choose the perfect package for your Debut</p>
    ...
    <div class="container-fluid py-6 wow fadeInUp" data-wow-delay="0.3s">
        <div class="container">
            <div class="text-center mb-5">
                ...
                <p class="fs-5 text-muted">Build your own debut package by selecting from our menu items below.</p>
            </div>
            <form id="customDebutForm">
                <div class="row">
                    <div class="col-lg-8">
    ...
    <div class="modal fade" id="bookingModal" tabindex="-1" aria-labelledby="bookingModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="bookingModalLabel">Book Your Debut Catering</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            bookingModal.addEventListener("shown.bs.modal", function (event) {
                ...
                // Logic for Custom vs Standard Package
                if (packageId == customPackageId || packageName === 'Custom Debut Package') {
                    guestsInput.value = numberOfGuests && numberOfGuests !== 'null' ? numberOfGuests : '';

                    // *** CRITICAL FIX: Ensure it's not read-only ***
                    ...
                    if (!pricePerHeadInput.value || pricePerHeadInput.value == '0') pricePerHeadInput.value = 100; // Default Price Per Head
                    pricePerHeadInput.readOnly = true;
                } else {
                    guestsInput.value = numberOfGuests;

                    // *** CRITICAL FIX: Ensure standard packages are read-only ***
                ...
                if (!calendarInitialized) {
                    ...
                    // Timezone correction logic from wedding.php
                    const now = new Date();
        ...
        // Client-side Pagination for Debut Packages (Structure adapted from Wedding.php)
        ...
        const debutMenus = <?php echo $debut_menus_json; ?>;
        ...
        // Show package details (Function copied from Wedding.php)
        ...
        $('#customBookNowBtn').on('click', function() {
            ...
            bookNowButton.setAttribute('data-package-name', 'Custom Debut Package');
            // For custom packages, the base price is 0
            bookNowButton.setAttribute('data-price', 0); 
            ...
            // FIXED: PASS MENU COST HERE
            bookNowButton.setAttribute('data-customization-total', menuTotalCost); 
        ...
        function updateCustomTotal() {
            ...
            // FIXED: KUHAIN ANG MENU TOTAL DITO MULA SA HIDDEN INPUT
            const menuTotal = parseFloat($('#modalCustomizationTotal').val()) || 0; 
        ...
        $(document).on('input', '#modalNumberOfGuests', function() {
            // Tiyakin na ang custom package lang ang ina-update
