...
// 2. Fetch Children's Party packages
try {
    $stmt = $db->prepare("SELECT * FROM catering_packages WHERE category = 'Childrens Party Catering'");
    $stmt->execute();
    $childrens_party_packages = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $packages_json = json_encode($childrens_party_packages);

    $itemsPerPage = 2;
    $totalPackages = count($childrens_party_packages);
...
// 3. Fetch the package_id for Custom Children's Party Package
$custom_package_id = null;
foreach ($childrens_party_packages as $pkg) {
...
// 4. Fetch active children's menu items for custom package
try {
    $menus_stmt = $db->prepare("SELECT m.*, c.category_name FROM menus m LEFT JOIN menu_categories c ON m.category_id = c.category_id WHERE m.service_type = 'childrens' AND m.status = 'active' ORDER BY c.category_name, m.title");
    $menus_stmt->execute();
    $childrens_menus = $menus_stmt->fetchAll(PDO::FETCH_ASSOC);

    // Fix image paths
    foreach ($childrens_menus as &$menu) {
    ...
    unset($menu);
    $childrens_menus_json = json_encode($childrens_menus);
} catch (PDOException $e) {
    $childrens_menus = [];
    $childrens_menus_json = json_encode([]);
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        /* --- COPYING STYLES FROM WEDDING.PHP (With Children's Party Colors) --- */
        ...
        .custom-menu-price {
            color: #fd7e14; /* Orange for Children's Theme */
            font-weight: 700;
            font-size: 1.1rem;
        }
        .custom-qty-group {
        ...
        /* CHILDREN'S PARTY THEME COLOR (Orange/Yellow Gradient) */
        .package-header {
...
<?php include '../layout/navbar.php'; ?>
        ...
        <div class="container">
            <div class="row g-5 align-items-center">
                <div class="col-lg-6">
                    ...
                    <div class="row g-4">
                        <div class="col-md-6"><div class="d-flex align-items-center mb-3"><i class="fas fa-check text-primary me-3"></i><span>Kid-Friendly Menus</span></div></div>
                        <div class="col-md-6"><div class="d-flex align-items-center mb-3"><i class="fas fa-check text-primary me-3"></i><span>Fun Presentations</span></div></div>
                        <div class="col-md-6"><div class="d-flex align-items-center mb-3"><i class="fas fa-check text-primary me-3"></i><span>Themed Setups</span></div></div>
    ...
    <div class="container-fluid py-6 wow fadeInUp" data-wow-delay="0.3s">
        <div class="container">
            <div class="text-center mb-5 wow fadeInUp" data-wow-delay="0.3s">
                <h1 class="display-4 text-title fw-bold">Children's Party Packages</h1>
                <p class="fs-5 text-subtitle">Choose the perfect package for the little ones</p>
                <p class="text-muted">Select a default package or customize it to fit your needs</p>
            </div>
    ...
    <div class="container-fluid py-6 wow fadeInUp" data-wow-delay="0.3s">
        <div class="container">
            <div class="text-center mb-5">
                ...
                <p class="fs-5 text-muted">Build your own party package by selecting from our kid-friendly menu items below.</p>
            </div>
            <form id="customChildrensForm">
                <div class="row">
                    <div class="col-lg-8">
    ...
    <div class="modal fade" id="bookingModal" tabindex="-1" aria-labelledby="bookingModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="bookingModalLabel">Book Your Children's Party</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
    ...
    <script>
        ...
        const childrensMenus = <?php echo $childrens_menus_json; ?>;
        ...
        document.addEventListener("DOMContentLoaded", function () {
            ...
            bookingModal.addEventListener("shown.bs.modal", function (event) {
                ...
                // Check if package is custom (either by ID or name check)
        ...
        function updateCartCount() {
            const totalItems = Object.values(customSelections).reduce((sum, sel) => sum + sel.quantity, 0);
        ...
        $('#customBookNowBtn').on('click', function() {
            ...
            bookNowButton.setAttribute('data-package-name', "Custom Children's Party Package");
